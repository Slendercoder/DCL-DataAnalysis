...
WSpred <- function(i, s, wAll, wNoth, wLef, wIn, alpha, beta, gamma, delta, epsilon, zeta, eta, regiones){
...
WSpredSL <- function(i, s, wAll, wNoth, wLef, wIn, alpha, beta, gamma, delta, epsilon, zeta, eta, regiones){
  aux <- c(wAll, wNoth, wLef, wLef, wLef, wLef, wIn, wIn)
  #  aux <- rep(w, 8)
  # The probability of region 'RS' is 1 - the sum of the other probabilities
  if (sum(aux) > 1) {
...
WSutil <- function(theta, args, regiones){
  ...
  args$probs <- lapply(args$pair, function(x) {
    i <- as.character(x[[1]][1])
    s <- as.numeric(x[[2]][1])
    return(WSpred(i, s, wAll, wNoth, wLef, wIn, alpha, beta, gamma, delta, epsilon, zeta, eta, regiones))
