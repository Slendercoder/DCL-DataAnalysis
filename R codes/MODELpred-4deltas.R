...
lower_limits=c(0,0,0,0,
               99,99,0,
               0,99,0.5,0,0,0)
upper_limits=c(0.25,0.25,0.25,0.25,
               100,100,32,
...
random_params <- function(x1, x2, x3, x4, 
  ...
  x11 <- c(1, x11)
  #  print(x11)
  a11 <- do.call(runif, as.list(x11))

  x12 <- unlist(x12)
  x12 <- c(1, x12)
  #  print(x12)
  a12 <- do.call(runif, as.list(x12))

  x13 <- unlist(x13)
  x13 <- c(1, x13)
  #  print(x13)
  a13 <- do.call(runif, as.list(x13))
...
FRApred1 <- function(i, iV, s, j, FRASims, theta) {
  ...
  # Add FRASim
  FRASims0 <- delta * sigmoid(unlist(FRASims[1]), epsilon, zeta)
  FRASims1 <- delta1 * sigmoid(unlist(FRASims[2]), epsilon, zeta)
  FRASims2 <- delta2 * sigmoid(unlist(FRASims[3:6]), epsilon, zeta)
  FRASims3 <- delta3 * sigmoid(unlist(FRASims[7:8]), epsilon, zeta)
  similarities <- c(0, unlist(FRASims0), unlist(FRASims1),
                    unlist(FRASims2), unlist(FRASims3))
